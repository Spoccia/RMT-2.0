...
for i = inputRange(1): inputRange(2)
    ...
    for j = 1: size(fileList, 2)
        ...
        XuniqueFeature = load ([trainPath, '/uniqueFeature_Class_', num2str(clusterID), '_',  num2str(j), '.csv']);
        Ximportance = load([trainPath, '/importance_Class_', num2str(clusterID), '_',  num2str(j), '.csv']);
        Xproject = load([trainPath, '/projectMatrix_Class_', num2str(clusterID), '_',  num2str(j), '.csv']);
        XdescrRange = load([trainPath, '/descrRange_Class_', num2str(clusterID), '_',  num2str(j), '.csv']);
