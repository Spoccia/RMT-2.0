        ...
        int nin, const mxArray *in[])
{
     ...
     ** --------------------------------------------------------------- */
    {
        ...
        for(k1 = 0 ; k1 < K1 ; ++k1, L1_pt += ND, Feature1_pt += NDFeature1,amp1_pt += 1)
        {
            ...
            int maxTime = max( (maxTime1), (maxTime2));
            ...
            for(k2 =  0 ; k2 < K2 ; ++k2, L2_pt += ND, Feature2_pt += NDFeature1, amp2_pt += 1)
            {
                ...
                double timeScoreOverlap=(min(te1, te2)-max(ts1, ts2))/(max(te1, te2)-min(ts1, ts2));
                double tempAbs = tcenter1 - tcenter2 ;
                
                double timeCenterProximity=0;
                if(maxTime == minTime)
                    timeCenterProximity = 0;
                else
                    timeCenterProximity=abs( tempAbs )/( maxTime - minTime);
                timeScoreOverlap = max(timeScoreOverlap, 0);
                timeCenterProximity = 1 - timeCenterProximity;
                
                
                double timeAlignment = (timeScoreOverlap + timeCenterProximity)/2;
                
//                 
//                 
//                 /*re-define timeScore without time overlap*/
//                 double avgLength = ((maxTime1 - minTime1) + (maxTime2 - minTime2))/2;// average length of matched-feature pair
//                 double timeCenterDiff = abs(tempAbs);
//                 double tempMaxTimeCD = max(te1, te2);
//                 double tempMinTimeCD = min(ts1, ts2);
//                 
//                 
//                 double timeScore = timeScoreOverlap;
                
                double tempAmpValue = amp1Value - amp2Value;
                double ampSimlarity = 1/(1 + abs(tempAmpValue));
                
                double matchingScore = acc * timeAlignment;
                double pruneScore = acc * timeAlignment;
