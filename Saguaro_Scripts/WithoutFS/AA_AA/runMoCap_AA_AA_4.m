...
saveFolder = ['./save_AA_AA_Folder_', num2str(scriptIndex), '/'];
...
for i = inputRange(1): inputRange(2)
    ...
    for j = 1: size(fileList, 2)
        ...
        [Dist(i,j), remainOctave] = DistanceFrames_AA_AA_a2p2(gss11, frame11, idm11, gss22, frame22, idm22, descr11, descr22,maxTime1, minTime1, maxTime2, minTime2,Xamp1,Xamp2);
        time(j) = toc(p);
        if(i ~= j)
