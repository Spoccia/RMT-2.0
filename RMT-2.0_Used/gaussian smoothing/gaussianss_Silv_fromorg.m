         ...
         if((otact==1)&&(odact==1))
            ...
            %(SS, CurrentTimeOct, CurrentDepdOct, ktime, kdepd,stmin,sdmin, stmax, sdmax,sigmaTscaleStep,sigmaDscaleStep,depd, Smatrix,soT,soD)
             ...
             if   ((otact ==1)&&(odact ~=1 )) %octave with octavetime=1
                ...
                H = dependencyALL(DistM , DepThreshold, IDM, odcur);
%                 Smatrix = ComputeDependencyScale(H, SDepdsigmafor_OT1_OD1); 
...
%             SS.smoothmatrix{otact, odact}(:,:,1,1)=Smatrix;
...
for sd=sdmin:sdmax 
    ...
    if sd==(sdmin)
        ...
        Smatrix = ComputeDependencyScale(depd{CurrentDepdOct}, dsigmaD);%ComputeDependencyScale(SS.smoothmatrix{CurrentTimeOct,CurrentDepdOct}(:,:,sd+soD-1), dsigmaD);% 
