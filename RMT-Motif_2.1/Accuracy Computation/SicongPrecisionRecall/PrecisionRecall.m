...
%         Path=['D:\Motif_Results\Datasets\SynteticDataset\',Ds_Name,'\',Ds_Name,' Motifs 1 2 3 same variate multisize\'];
        %RandomVariate\instancessamesize\'];%Coherent Shift Variate 1M ',Ds_Name,'\instancesmultisize'];%'\samesize10inst'];%'\BirdSong M 1 2 3'];%'\numInstances_5_15'];
    ...
    TS_index = AllTS(1:4);%sort(AllTS(1:30));
    for overlapid=1:2
        overlapping='';%
        if overlapid==2
...
%            BaseName='Motif10';%'Motif1numInst_10';%'MV_Sync_Motif1';%%1';%'Motif';%
