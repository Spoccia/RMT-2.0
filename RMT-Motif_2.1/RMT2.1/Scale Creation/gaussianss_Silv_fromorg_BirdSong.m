...
%do a smoothing as:
STimegsigmafor_OT1_OD1 = sigmaT0;%sqrt((sigmaT0*ktime^sminT)^2  - (sigmaNT/2^ominT)^2);%
         ...
         if((otact==1)&&(odact==1))
            ...
            %(SS, CurrentTimeOct, CurrentDepdOct, ktime, kdepd,stmin,sdmin, stmax, sdmax,sigmaTscaleStep,sigmaDscaleStep,depd, Smatrix,soT,soD)
             ...
             if   ((otact ==1)&&(odact ~=1 )) %octave with octavetime=1
                ...
                [SS.octave{otact,odact}(:,:,1,1)] = HalfDependencyMote_BirdSong(LocMTemp, squeeze(SS.octave{otact,odact-1}(:,:,sbest_Dep+soD,1)),IDMall{odact}); 
                %distance matrix
...
%                 H = dependencyALL(DistM , DepThreshold, IDM, odcur);
             elseif ((otact ~=1)&&(odact==1)) %octave with octave dependency 1
                ...
                [SS.octave{otact,odact}(:,:,1,1)] = HalfDependencyMote_BirdSong(LocMTemp, squeeze(TMP),IDMall{odact});
%                 DistM = computeDist(LocMTemp, IDM, odcur);
                %% Silv Normalization
%                     maxim = max(DistM(:));
%                     minim = min(DistM(:));
...
%                 H = dependencyALL(DistM , DepThreshold, IDM, odcur);
            ...
            SS = Smooth_Asyn(SS, otact, odact, ktime, kdepd, sminT, sminD,smaxT, smaxD ,dsigmaT0, dsigmaD0,        depd,    Smatrix,soT,soD);% ComputeDependencyScale(depd{odact}, sigmaD0),soT,soD);% 
...
for sd=sdmin:sdmax 
    ...
    if sd==(sdmin)
        ...
        Smatrix = ComputeDependencyScale(depd{CurrentDepdOct}, dsigmaD);%ComputeDependencyScale(Smatrix, dsigmaD);%
