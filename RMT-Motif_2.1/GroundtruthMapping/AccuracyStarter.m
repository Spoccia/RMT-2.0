...
for DSIdx =2:2
    ...
    %Ds_Name,'\Coherent Shift Variate 1M ',Ds_Name,'\instancessamesize\'];%RandomVariate\instancessamesize\'];%BirdSong\samesize10inst\';%Coherent Shift Variate 1M Energy\instancessamesize\';%
...
%     BaseName=['Motif1numInst_',num2str(possnuminst(indpossnuminst))]; %'MV_Sync_Motif1';%'MV_Sync_Motif1';%
    NO_V_subcluster=1; % 1 subvariateoff %  0;%0 subvairate on %
    dataLocation='data\';
    ...
    for idmotInj =1: 1%size(PossibleMotifInjected,2)-1
        ...
        for percentid=1:size(percent,1)
            ...
            DepO =2;
            DepT =2;
            for strID =6:2:6%size(strategy,2)%1:size(strategy,2)%1:size(strategy,2)%9%5:6%4:6%4:6%1:size(strategy,2)
