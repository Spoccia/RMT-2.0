...
featuresToInjectPath=[basepath,DataType,'\',DataType,'RandomShift\MultiSize\data\FeaturesToInject\'];
% 'CoherentShift\SameSize\data\FeaturesToInject\'];%'CoherentShift\MultiSize\data\FeaturesToInject\'];%

randomWalkPath = [basepath,DataType,'\',DataType,'RandomShift\MultiSize\data\RW_0_1/RW_'];
%'CoherentShift\SameSize\data\RW_0_1/RW_'];%'CoherentShift\MultiSize\data\RW_0_1/RW_'];%

TimeSeriesPath = ['D:\Motif_Results\Datasets\',DataType,'\data\'];%['/Users/sliu104/Desktop/Motif_Data/TimeSeries/',DataType,'/data/'];
depdO=2;
num_of_motif=1;
delimiter='/';
DestDataPath = [basepath,DataType,'\',DataType,'RandomShift\MultiSize\data\'];%'CoherentShift\SameSize\data\'];%'CoherentShift\MultiSize\data\'];%
...
%                     'Controlla il file  per mocap per le variate random'
%                     pause;
...
NormInterval=[zeros(NUM_VARIATE,1),ones(NUM_VARIATE,1)];
                ...
                for motifInstance = 1: motif_instances*num_of_motif
                    ...
                    for varaitetouse=1:size(listvariates)
                        ...
                        RWShiftedvalue = RwtoShift(variatesofInjection(varaitetouse),1)- MotifInstanceShifted(listvariates(varaitetouse),end);
                        RwtoShift(variatesofInjection(varaitetouse),:)= RwtoShift(variatesofInjection(varaitetouse),:) - RWShiftedvalue;
                    end
                    ...
                    Motif1RW(actualVariateToinject((actualVariateToinject(:,1)>0),1),starterTime(motifInstance):starterTime(motifInstance)+scalingTime-1) = ...
                        MotifInstanceShifted(DepdToInject(DepdToInject(:,1)>0,1),:);% M1(actualVariateToinject(actualVariateToinject(:,1)>0,1),:);
                    
                    Motif1RW(actualVariateToinject((actualVariateToinject(:,1)>0),1),starterTime(motifInstance)+scalingTime:TotalRWlength)= ...
                        RwtoShift(actualVariateToinject((actualVariateToinject(:,1)>0),1),2:end);%2:end-scalingTime);
