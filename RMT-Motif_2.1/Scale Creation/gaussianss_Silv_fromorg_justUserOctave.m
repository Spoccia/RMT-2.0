function [ SS,depd, IDMall,TIMESCALE] = gaussianss_Silv_fromorg_justUserOctave(I,LocM,Ot,Od,St,Sd,ominT,ominD,sminT,sminD,smaxD,smaxT,sigmaT0,sigmaD0,DepThreshold,dsigmaT0,dsigmaD0)%
...
doubleSigmaTime = sigmaT0*2;
doubleSigmaDepd = sigmaD0*2;
...
while(~isover)
    if(otact<Ot & odact<Od)
        ...
        [SS.octave{otact,odact}(:,:,sd,st),SS.smoothmatrix{otcur,odcur}(:,:,sd,sd)] = smooth(SS.smoothmatrix{otact,odact}(:,:,1,1), Smatrix, doubleSigmaTime);
...
%         smooth of 2 sigmatime over time
    [SS.octave{otact,odact}(:,:,1,ST)] = smoothJustTimeSilv(TMP, sqrt(target_sigmaT^2 - prev_sigmaT^2),Smatrix ) ;
