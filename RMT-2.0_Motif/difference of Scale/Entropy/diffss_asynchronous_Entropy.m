function dss = diffss_asynchronous_Entropy(ss)
dss.otmin = ss.otmin;
dss.odmin = ss.odmin ;
dss.Od = ss.Od ;
dss.Ot = ss.Ot;
...
for ODepd = 1 : size(ss.octave,2)
    for OTime = 1 : size(ss.octave,1)
        ...
        for i = 1: (Depd-1)
            RowVector(:, :, i) = ss.Entropyoctave{OTime, ODepd}( :, :, i+1, i)-ss.Entropyoctave{OTime, ODepd}( : ,:, i, i);
        end
        
        for i = 1 : (Time-1)
            ColumnVector(:,:,i) = ss.Entropyoctave{OTime, ODepd}( :, :, i, i+1)-ss.Entropyoctave{OTime, ODepd}( : ,:, i, i);
        end
        
        for i = 1 : (Depd-1)
            % DiaVectorDown(:,:,i) = ss.octave{OTime, ODepd}( :, :, i+1, 1)-ss.octave{OTime, ODepd}( : ,:, i, 1);
            DiaVectorDown(:,:,i) = ss.Entropyoctave{OTime, ODepd}( :, :, i+1, i+1)-ss.Entropyoctave{OTime, ODepd}( : ,:, i, i);
        end
        
        dss.Entropyoctave{OTime, ODepd}{1} = RowVector; % depd difference
        % dss.octave{OTime, ODepd}{1} = reshape(RowVector, M, N, Time*(Depd-1));
        dss.Entropyoctave{OTime, ODepd}{2} = ColumnVector; % time difference
        % dss.octave{OTime, ODepd}{2} = reshape(ColumnVector, M, N, Time*(Depd-1));
        dss.Entropyoctave{OTime, ODepd}{3} = DiaVectorDown; % both directions
