...
SizeofK= 'Akmeans_mean2';%'Fixed';%'Threshould';% 'Computed';%'Fixed_K15';%
K_valuesCalc=SizeofK;
CleaningFeatures ='';
KindofFeatures= 0; % 1 for DoG 0 for DoE

for TSnumber = 4:4 %100: 184
...
%                         [C,mu] = cvKmeansCombined(X, DictionarySizeApplied,KmedoidsCoefTerm ,'@Distance_RMT_DESC_AMP',false,data,gss1,idm1,KmeansDescmetric);
                    elseif(strcmp(typeofCluster,'Cluster_AKmeans')==1)
                        [C,mu,inertia,tryK,startK]= adaptiveKmeans(X,3,0.005,0,'sqeuclidean');%0 will fix the step to 2 as default
%                       [C,mu,inertia,tryK,startK]= adaptiveKmeans(X,3,0.1,2,'sqeuclidean');
% (X,3,0.02,2,'sqeuclidean');
                       % TotClusterTime_0 = TotClusterTime_0 - toc;
                       TotClusterTime_0 = toc;
...
%     end
    if(motifidentificationBP_MatlabDescr ==1)
        ShowKmeansCluster(TS_name,datasetPath,subfolderPath,TS_name,K_valuesCalc,distanceUsed,typeofCluster,histTSImage,FeaturesRM,0);
        ShowVaraiteallineadCluster(TS_name,datasetPath,subfolderPath,TS_name,K_valuesCalc,distanceUsed,typeofCluster,histTSImage,FeaturesRM,0);
    end
    
    % Prune the clusters
    if(pruneClusterDescrMatlab==1)
        TimeforPruningClustering = KmeansPruning(TS_name,datasetPath,subfolderPath,TS_name,typeofCluster,K_valuesCalc,prunewith,distanceUsed ,DictionarySize,histTSImage,FeaturesRM,CleaningFeatures,1);
        TimeforPruningSubClustering = VariateAllinedKmeansPruning(TS_name,datasetPath,subfolderPath,TS_name,typeofCluster,K_valuesCalc,prunewith,distanceUsed ,DictionarySize,histTSImage,FeaturesRM,CleaningFeatures,1);
    end
