function [tempFrames,descriptors,gss,dogss,depd,idm, time, timee, timeDescr]=sift_gaussianSmooth_Silv_Xiaolan(I, LocM, Ot, Od, St, Sd, sigmaTime ,sigmaDepd, NBP, gthresh, r,sBoundary, eBoundary)
% [M,N,C] = size(I) ;
...
[gss, depd, idm] = gaussianss_Silv_fromorg_Xiaolan(I, LocM, Ot,Od,St,Sd,otmin,odmin,stmin,sdmin,St+1,Sd+1, sigmaTime, sigmaDepd,gthresh,-1,-1);

%[gss, depd, idm] = gaussianss_asynchronousMote_Silv_2(I, LocM, Ot, Od,St, Sd, sigmaTime, sigmaDepd, gthresh);
timee(1) = timee(1)+ toc(p);

p = tic;
dogss = diffss_asynchronous_Xiaolan(gss);
...
for otime = 1: size(gss.octave,1)
    ...
    scaleDiff = St - 1;
    forwardIdx = siftlocalmax_directed_100(dogss.octave{otime, odepd}, 0.8*thresh, NormalizeF(depd{odepd}), NormalizeB(depd{odepd}'), scaleDiff);
    
    [i,j, s1] = ind2sub( size( dogss.octave{otime, odepd}), forwardIdx ) ;
    y=i-1;
    x=j-1;
    s1=s1-1+gss.sminT;
    % s1=s1-1-1 ; SICONG
    ...
    dogss.octave{otime, odepd} = -dogss.octave{otime, odepd};
    
    backwardIdx = siftlocalmax_directed_100(dogss.octave{otime, odepd}, 0.8*thresh, NormalizeF(depd{odepd}), NormalizeB(depd{odepd}'), scaleDiff);
    ...
    oframes = C';
    for hsize = 1:size(dogss.octave{otime, odepd},3)% iterate on the scale in the specific octave
        HY(:,:,hsize) = (NormalizeByRow(depd{odepd})*(-dogss.octave{otime, odepd}(:,:,hsize)'))';
        HY2(:,:,hsize) = (NormalizeByRow(depd{odepd}')*(-dogss.octave{otime, odepd}(:,:,hsize)'))';
    end
    rad = 0;
    ...
    %   ofra =oframes;
    oframes = siftrefinemx_directed(oframes, -dogss.octave{otime, odepd},HY,HY2,gss.sminT,thresh,r,0) ;
