function [TimeforPruningSubClustering ] = VariateAllinedKmeansPruning(TEST, imagepath,specificimagepath,imagename,typeofCluster,K_valuesCalc,prunewith,distanceUsed ,DictionarySize,histdataimage,FeaturesRM,cleanfeatures,saveMotifImages )
...
%              if(abs(size(X,2))>=DictionarySizeApplied & abs(size(X,2))>0)
                ...
                %csvread(strcat(saveFeaturesPath,'DistancesDescriptors\DepdScale_IM_',imagename,'_DepO_',num2str(j),'_TimeO_',num2str(k),'.csv'));%_',imagename,'_',num2str(p),'.csv'));
...
%                     end
%                     if(size(A1,2)>1)
%                         if(exist([ImageSavingPath,'/OT_',num2str(k),'_OD_',num2str(j)],'dir')==0)
%                             mkdir([ImageSavingPath,'/OT_',num2str(k),'_OD_',num2str(j),'\']);
%                         end
...
%                         
%                         imwrite(uint8(dataid(:,:,i)),[ImageSavingPath,'/OT_',num2str(k),'_OD_',num2str(j),'\Im_',imagename,'_octT_',num2str(k),'_octD_',num2str(j),'_Cl_',num2str(i),'.jpg'])
%                         imwrite(uint8(histdataid(:,:,i)),[ImageSavingPath,'/OT_',num2str(k),'_OD_',num2str(j),'\HistIm_',imagename,'_octT_',num2str(k),'_octD_',num2str(j),'_Cl_',num2str(i),'.jpg'])
%                         imwrite(Combined,[ImageSavingPath,'/OT_',num2str(k),'_OD_',num2str(j),'\CHistIm_',imagename,'_octT_',num2str(k),'_octD_',num2str(j),'_Cl_',num2str(i),'.jpg'])
%                     end
%                     
                end
