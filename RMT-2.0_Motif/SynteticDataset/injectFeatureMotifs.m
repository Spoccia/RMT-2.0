...
FeaturePath = 'D:\Motif_Results\Datasets\Building_MultiStory\Features_RMT\1_Bsigma\';
%'D:\githubGmail\RMT-2.0\RMT-2.0_Motif\SynteticDataset\EnergyDS\1_norm\';
...
%% normalize the random walk in the interval of the  data series from with we estracted the motif
% if(strcmp(kindofBasicTS,'randomWalk')==1)
%      rndWalks = rndWalkGeneration(size(data,1),size(data,2)); %%geerate random walk z-normalized
...
%     FeatPositions(ii,:)=[ceil(i/numFeatureforClass),i,starter,starter+motifclmn-1];
    ...
    if(strcmp(kindofBasicTS,'randomWalk')==1 | strcmp(kindofBasicTS,'flat')==1)
        rndWalks(B((B(:,i)>0),i),starter:starter+motifclmn-1) = motifData(B(B(:,i)>0,i),:);%data(B(B(:,i)>0,i),intervaltime((intervaltime>0 & intervaltime<=size(data,2))));
    elseif(strcmp(kindofBasicTS,'Sinusoidal')==1)       
       rndWalks(B((B(:,i)>0),i),starter:starter+motifclmn-1)  = motifData(B(B(:,i)>0,i),:);%data(B(B(:,i)>0,i),intervaltime((intervaltime>0 & intervaltime<=size(data,2))));       %rndWalks(B((B(:,i)>0),i),starter:starter+motifclmn-1) + data(B(B(:,i)>0,i),intervaltime((intervaltime>0 & intervaltime<=size(data,2))));       
    end
    pStep=pStep+Step;
end
% end
