    ...
    for i = 1 : size(num_of_motif, 2)
        for j = 1 : size(strategy, 2)
            for k = 1 : size(amp_scale, 2)
                ...
                fprintf('num_of_motif: %d, strategy: %d, algorithm_type: %s, amp_scale: %f, time overlap threshold: %f . \n', cur_num_of_motif, cur_strategy, cur_algorithm_type, amp_scale(k), timeOverlapThreshold);
                
                sharedFolder = ['/Users/sliu104/Desktop/Results_Motif_Save_NewlyAdded/Motif', num2str(cur_num_of_motif), '/Strategy_', num2str(cur_strategy), '/amp_scale_', num2str(amp_scale(k)), '_TO_', num2str(timeOverlapThreshold)];
