...
strategy = 3;
num_of_motif = [1];
algorithm_type = ['MStamp']; % MatrixProfile RMT RME cleanmatlabentropy09
% timeOverlapThresholds = [0.1, 0.25, 0.5, 0.75, 1];
timeOverlapThresholds = [1];
amp_scale = 0;
    ...
    for i = 1 : size(num_of_motif, 2)
        for j = 1 : size(strategy, 2)
            ...
            for aa = 1 : size(amp_scale, 2)
                ...
                sharedFolder = ['/Users/sicongliu/Desktop/Results_Motif_Save_NewlyAdded/', algorithm_type, '/Motif', num2str(cur_num_of_motif), '/Strategy_', num2str(cur_strategy), '/amp_scale_', num2str(amp_scale(aa)), '_TO_', num2str(timeOverlapThreshold)];
