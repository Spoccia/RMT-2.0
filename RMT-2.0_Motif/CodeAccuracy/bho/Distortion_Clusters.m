...
% Feature_Injected = csvread([path,kindofinj,'IndexEmbeddedFeatures\FeaturesEmbedded_',TEST,'.csv']);%',TEST,'\
...
row_header =[];
    SpaceDistances= pdist2(Featurepruned(11:138,:)',Featurepruned(11:138,:)','cosine');
     maximumSpaceDistance= 1;%max(SpaceDistances(:));
for i=1: nCluster
    ...
    DescriptorCluster = mean(F(11:end,:)');
...
Distortion = pdist2(F(11:138,:)',F(11:138,:)','cosine')/maximumSpaceDistance;%,'cosine')/2;
 DistortionCluster = pdist2(DescriptorCluster,F(11:138,:)','cosine')/maximumSpaceDistance;
%DistortionCluster = pdist2(Centroidpruned(:,i)',F(11:138,:)','cosine')/maximumSpaceDistance;%,'cosine')/2;%);%
