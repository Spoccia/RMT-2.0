function [Cluster Codebook] = cvKmeansCombined(X, K, stopIter, distFunction, verbose,TS,gss1,idm1,KmeansDescmetric)
    ...
    % [C(1,:), index(1)] = datasample(S,X',1);
     ...
     XAMP=amplitudediff(TS,X,gss1,idm1);
     if (size(X,2)==1)
      initialindex=1;
    ...
    iter
       ...
       %     [distmedianchk,minimochk]= min(sum(abs(actdesc-avdes)));%averageDesc)));
            ...
            distlocAmp = abs((XampCluster_i - averageAmp)./(XampCluster_i + averageAmp));
            simDistDescr= 1./(distlocdesc+1);
            simDistAmp = 1./(distlocAmp+1);
            DistFromSim= ones(1,size(simDistDescr,2)) - (simDistDescr.*simDistAmp);
            ...
            [distmedian,minimo]= min(DistFromSim(:,1));%abs(actdesc-avdes)));%averageDesc)));
