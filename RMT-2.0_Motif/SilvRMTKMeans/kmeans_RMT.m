function [idxbest, Cbest, sumDbest, Dbest] = kmeans_RMT(X, k, varargin)
...
%   vector, KMEANS treats it as an N-by-1 data matri, regardless of its
...
%   the K-by-P matrix C.
%
%   [IDX, C, SUMD] = KMEANS(X, K) returns the within-cluster sums oxf
    ...
    function cellout = loopBody(rep,S)
        ...
        switch start
            ...
            case {'plus','kmeans++'}
                ...
                for ii = 2:k        
                    distanza=distfun(X,C(1:ii-1,:),distance);
