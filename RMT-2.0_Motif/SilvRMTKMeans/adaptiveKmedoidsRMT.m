function  [C1,mu1,inertia,tryK,startK]=adaptiveKmedoidsRMT(features1,K_start,saturation,Step)%,distance)
isFound=false;
inertia=[];
%    AllFeaturesDistances = pdist2(features1(11:end,:)',features1(11:end,:)');
maxdistance2instances= 1;%max(AllFeaturesDistances(:));
if Step==0
    Step=2;
end
itr=1;
...
while(~isFound)
    ProposedC1=C1;
    Proposedmu=mu1;
    [C1,mu1,SUMD, D]=KmedoidsRMT(features1,startK,  false);
    %       kmeans(features1(11:size(features1,1),:)',startK,'Distance',distance,'Replicates',5);%'Display','final'
...
% % %     MeasureToUse=mean3;%sum(SUMD2);%mean(mean3);%MeanD2);% sum(SUMD2);%
