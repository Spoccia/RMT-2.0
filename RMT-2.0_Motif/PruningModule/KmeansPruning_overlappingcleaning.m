function [  ] = KmeansPruning_overlappingcleaning(TEST, imagepath,specificimagepath,imagename,typeofCluster,K_valuesCalc,prunewith,distanceUsed ,DictionarySize,histdataimage,FeaturesRM )
           ...
           X=frame1(:,indexfeatureGroup);
            ...
            if(abs(size(X,2))>=DictionarySizeApplied & abs(size(X,2))>0)
                ...
                for i=1:nCluster
                    ...
                    [A,B]=pruneoverlappingFeaturesClustered(A,B,Centroid_desriptor,overTime,overDep);
...
%                     [A,B]=pruneoverlappingFeaturesClustered(A,B,Centroid_desriptor,overTime,overDep);
                    ...
                    for iii=1: size(A1,2)
                        ...
                        FeatureLocation(min(B1((B1(:,iii)>0),iii)),(Xs):(Xe),1,i)= Starting(min(B1((B1(:,iii)>0),iii)),(Xs):(Xe))+1;
                        FeatureLocation(B1((B1(:,iii)>0),iii),Xe,2,i)= Ending(B1((B1(:,iii)>0),iii),Xe)+1;
                        FeatureLocation(max(B1((B1(:,iii)>0),iii)),(Xs):(Xe),2,i)= Ending(max(B1((B1(:,iii)>0),iii)),(Xs):(Xe))+1;
