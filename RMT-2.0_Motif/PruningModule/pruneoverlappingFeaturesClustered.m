function [A,B]=pruneoverlappingFeaturesClustered(A,dpscale,Centroid_desriptor,overTime,overDep)
    timescope= A(4,:)*3;
    iii=1;
    kindofCriteria=2;
    %% kindofcriteria:
    ...
    while iii <= size(A,2)
        ...
        while jjj <= size(A,2)
            ...
            if(size(intersect(timerangeF1,timerangeF2),2)>0)
                ...
                if((Time_inter_Score*Dep_inter_Score) >=(overTime*overDep))%(0.8*0.8))%(Time_inter_Score>= 0.8 & Dep_inter_Score >= 0.8)
